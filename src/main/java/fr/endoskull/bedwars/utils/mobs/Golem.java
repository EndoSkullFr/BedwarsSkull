...
public class Golem extends EntityIronGolem {
    ...
    private Golem(World world, Team team, Arena game) {
        super(world);
        this.team = team;
        this.arena = game;
        try {
        ...
        this.targetSelector.a(3, new PathfinderGoalNearestAttackableTarget(this, Golem.class, 20, true, false, golem -> {
            if (golem == null) return false;
            if (((Golem) golem).getTeam() == null) return false;
            return !((Golem)golem).getTeam().equals(team);
        }));
    ...
    public static LivingEntity spawn(Location loc, Team bedWarsTeam, Arena game, double speed, double health, int despawn) {
        WorldServer mcWorld = ((CraftWorld) loc.getWorld()).getHandle();
        Golem customEnt = new Golem(mcWorld, bedWarsTeam, game);
